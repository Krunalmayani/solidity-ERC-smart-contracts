...
contract CareCoin is  Ownable {
    ...
    function _getValues(uint256 tAmount) public  returns(uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _tokenFromReflection(uint256 rAmount) private  returns(uint256) {
        require(rAmount <= _rTotal,"Amount must be less than total reflections");
        uint256 currentRate = _getRate();
        return rAmount.div(currentRate);
    }

    function _getRate() private  returns(uint256) {
